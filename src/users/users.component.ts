...
@Component({
  selector: 'app-users',
  standalone: true,
  imports: [GridComponent, DatePipe, GridCellDirective],
  template: `
    <table app-grid [data]="data()" [columns]="columns()">
      <td *appCell="let row" cellKey="name">{{ row.name }}</td>
      <td *appCell="let row" cellKey="age">{{ row.age }}</td>
      <td *appCell="let row" cellKey="role">{{ row.role }}</td>
      <td *appCell="let row" cellKey="createdAt">
        {{ row.createdAt | date }}
      </td>
    </table>
  `,
